...
opc = {
    ...
    'Tipo de union': 'SW',     # [HYBRID] | ADS | SW | NONE
...
# Medidas en [m]
lado = 0.040 			# Lado del poligono base de la seccion. Default: 0.0667
aleta = 0.015 			# Ancho de las aletas sobre las que se unen las chapas. Default: 0.02
espesor = 0.001 		# Espesor de las chapas
longitud = 0.30 		# Longitud total del tubo. Default: 0.35
...
# DynProps
delta = 0.5*longitud 	# Desplazamiento. Default: 0.25
...
if opc['Host'] == 'BREOGAN':
    ...
    chapa_seed_size = 0.002 	# 0.002
    ads_seed_size = 0.001 		# 0.001
    ads_seed_nesp = 1 			# 1
    interv = 500 				# 500
elif opc['Host'] == 'LOCAL':
...
# Chapa
#------------------------------------------------------------------------------
def creaChapa(modelo=modelo, opc=opc):
    ...
    s.Line(point1=(ladoh/2, 0.0), 									point2=(ladoh/2 + ladov*seno, ladov*coseno - espesor/2))
    s.Line(point1=(ladoh/2 + ladoh*seno, ladov*coseno - espesor/2), point2=(ladoh/2 + ladoh*seno + aleta, ladov*coseno - espesor/2))
    ...
    if adherente == 'Al' and mats == 'PREDEF':
        ...
        mat_alum2.Plastic(hardening=ISOTROPIC, table=((190000.0, 0.0), (950000.0, 1.0)))
    ...
    p = modelo.parts['Chapa']
    chapa = a.Instance(name='Chapa-1', part=p, dependent=ON)
    ...
    a.rotate(instanceList=('Chapa-2', ),
    	axisPoint=(0.0, ladov*coseno, 0.0),
    	axisDirection=(0.0, 0.0, 1.0), angle=180.0)
...
if adhesivo == True:
	...
	if adhesive_name == 'Loctite Hysol 9514':
	    ...
	    matads.Elastic(type=TRACTION, table=((1460000.0, 563707.0, 563707.0), ))
	    ...
	    matads.quadsDamageInitiation.DamageEvolution(type=ENERGY, mixedModeBehavior=POWER_LAW, power=2.0, table=((905, 905, 905), )) # From Sadowski, among others
        ...
        intadsprops.Damage(criterion=QUAD_TRACTION, initTable=((9500.0, 40000.0, 40000.0), ), # From manufacturer specs for steel
	        useEvolution=ON, evolutionType=ENERGY, useMixedMode=ON, # Evolution needed (manual: w/o it, it's computed, but not applied)
	        mixedModeType=POWER_LAW, exponent=2.0,
	        evolTable=((2028., 11853., 11853.), ))  # Scattina
	...
	modelo.CohesiveSection(name='SecCoh_' + adhesive_name,
		material=adhesive_name, response=TRACTION_SEPARATION, outOfPlaneThickness=None)
 ...
 #    	offsetType=MIDDLE_SURFACE, offsetField='', thicknessAssignment=FROM_SECTION)
	...
	a.translate(instanceList=('Adhesivo-1', ), vector=(ladoh/2 + ladov*seno,		 ladov*coseno + opc['Espesor adhesivo']/2,	 0.0))
	a.translate(instanceList=('Adhesivo-2', ), vector=(-ladoh/2 -ladov*seno - aleta, ladov*coseno + opc['Espesor adhesivo']/2,	 0.0))
	a.rotate(instanceList=('Adhesivo-1', 'Adhesivo-2', ),
		#axisPoint=(0.0, ladov*coseno + opc['Espesor adhesivo'], 0.0),
		axisPoint=(0.0, ladov*coseno + opc['Espesor adhesivo']/2, 0.0),
...
a.translate(instanceList=('Placa-1', ), vector=(0.0, tan(30*pi/180)*lado/2, 0.0))
...
region1 = a.Set(faces=a.instances['Placa-1'].faces[0:1], name='b_Set-6')
...
region1 = a.Set(faces=a.instances['Placa-1-lin-2-1'].faces[0:1], name='b_Set-4')
...
if soldadura == True:
	...
	#datum1 = modelo.rootAssembly.datums[1]
	a.engineeringFeatures.PointFastener(targetSurfaces=(Ch1_u1, Ch1_u2, Ch2_u1, Ch2_u2, ),
...
if adhesivo == True:
	...
	surf_chapa_all = [	Ch2_int1, Ch2_int2, Ch1_int3, Ch1_int4, Ch1_int5, Ch2_int6, Ch1_u1, 	Ch1_u2, 	Ch2_u1, 	Ch2_u2,
						Ch2_ext1, Ch2_ext2, Ch1_ext3, Ch1_ext4, Ch1_ext5, Ch2_ext6, Ch1_ext_u1, Ch1_ext_u2, Ch2_ext_u1, Ch2_ext_u2]
	...
	# Adhesivo (asignacion individual)
	modelo.interactions['Global-Int'].contactPropertyAssignments.appendInStep(stepName='Initial', assignments=(
		(GLOBAL, SELF, 'Global-IntProp'),
		(Ads1_bot, Ch1_u1, intadhesive_name),
		(Ads1_top, Ch2_u1, intadhesive_name),
		(Ads2_bot, Ch1_u2, intadhesive_name),
		(Ads2_top, Ch2_u2, intadhesive_name),
		))
